...
import in.pft.apis.creditbazaar.gateway.repository.rowmapper.FundsTransferTransactionDetailsRowMapper;
...
/**
 * Spring Data R2DBC custom repository implementation for the FundsTransferTransactionDetails entity.
 */
@SuppressWarnings("unused")
class FundsTransferTransactionDetailsRepositoryInternalImpl
    extends SimpleR2dbcRepository<FundsTransferTransactionDetails, Long>
    implements FundsTransferTransactionDetailsRepositoryInternal {
    ...
    private final FundsTransferTransactionDetailsRowMapper fundstransfertransactiondetailsMapper;

    private static final Table entityTable = Table.aliased("funds_transfer_transaction_details", EntityManager.ENTITY_ALIAS);
    ...
    public FundsTransferTransactionDetailsRepositoryInternalImpl(
        ...
        FundsTransferTransactionDetailsRowMapper fundstransfertransactiondetailsMapper,
        R2dbcEntityOperations entityOperations,
        R2dbcConverter converter
    ) {
        super(
            new MappingRelationalEntityInformation(
                converter.getMappingContext().getRequiredPersistentEntity(FundsTransferTransactionDetails.class)
            ),
            entityOperations,
            converter
        );
    ...
    @Override
    public Flux<FundsTransferTransactionDetails> findAllBy(Pageable pageable) {
        return createQuery(pageable, null).all();
    }

    RowsFetchSpec<FundsTransferTransactionDetails> createQuery(Pageable pageable, Condition whereClause) {
        List<Expression> columns = FundsTransferTransactionDetailsSqlHelper.getColumns(entityTable, EntityManager.ENTITY_ALIAS);
        ...
        String select = entityManager.createSelect(selectFrom, FundsTransferTransactionDetails.class, pageable, whereClause);
        return db.sql(select).map(this::process);
    }

    @Override
    public Flux<FundsTransferTransactionDetails> findAll() {
        return findAllBy(null);
    }

    @Override
    public Mono<FundsTransferTransactionDetails> findById(Long id) {
        Comparison whereClause = Conditions.isEqual(entityTable.column("id"), Conditions.just(id.toString()));
        return createQuery(null, whereClause).one();
    }

    @Override
    public Mono<FundsTransferTransactionDetails> findOneWithEagerRelationships(Long id) {
        return findById(id);
    }

    @Override
    public Flux<FundsTransferTransactionDetails> findAllWithEagerRelationships() {
        return findAll();
    }

    @Override
    public Flux<FundsTransferTransactionDetails> findAllWithEagerRelationships(Pageable page) {
        return findAllBy(page);
    }

    private FundsTransferTransactionDetails process(Row row, RowMetadata metadata) {
        FundsTransferTransactionDetails entity = fundstransfertransactiondetailsMapper.apply(row, "e");
    ...
    @Override
    public <S extends FundsTransferTransactionDetails> Mono<S> save(S entity) {

...
@Repository
public interface RequestOfferRepository extends ReactiveCrudRepository<RequestOffer, Long>, RequestOfferRepositoryInternal {
    ...
    @Query("SELECT * FROM request_offer entity WHERE entity.cbcreprocess_id = :id")
    Flux<RequestOffer> findByCbcreprocess(Long id);

    @Query("SELECT * FROM request_offer entity WHERE entity.cbcreprocess_id IS NULL")
    Flux<RequestOffer> findAllWhereCbcreprocessIsNull();

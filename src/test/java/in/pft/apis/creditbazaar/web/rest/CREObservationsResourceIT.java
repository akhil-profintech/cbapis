...
import in.pft.apis.creditbazaar.repository.CREObservationsRepository;
import in.pft.apis.creditbazaar.repository.EntityManager;
import in.pft.apis.creditbazaar.service.CREObservationsService;
import in.pft.apis.creditbazaar.service.dto.CREObservationsDTO;
import in.pft.apis.creditbazaar.service.mapper.CREObservationsMapper;
...
class CREObservationsResourceIT {
    ...
    @Test
    void checkCreObservationsIdIsRequired() throws Exception {
        int databaseSizeBeforeTest = cREObservationsRepository.findAll().collectList().block().size();
        // set the field null
        cREObservations.setCreObservationsId(null);

        // Create the CREObservations, which fails.
        CREObservationsDTO cREObservationsDTO = cREObservationsMapper.toDto(cREObservations);
        ...
        List<CREObservations> cREObservationsList = cREObservationsRepository.findAll().collectList().block();
        assertThat(cREObservationsList).hasSize(databaseSizeBeforeTest);
    }
    ...
    void partialUpdateCREObservationsWithPatch() throws Exception {
        ...
        partialUpdatedCREObservations.creObservationsId(UPDATED_CRE_OBSERVATIONS_ID).creRequestId(UPDATED_CRE_REQUEST_ID);
